...
func Create(c *gin.Context) {
	...
	if len(user.CourseID) > 0 {
		for _, courseID := range user.CourseID {
			userCourse := new(models.UserCourse)
			userCourse.UserID = user.ID
			userCourse.CourseID = courseID
			models.DB.Create(&userCourse)
		}
		c.JSON(http.StatusOK, gin.H{"user": user})
...
func JoinCourse(c *gin.Context) {
	...
	if models.DB.Model(&course).Where("Key = ?", key).Updates(&user.Course).RowsAffected == 0 {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"massage": "data tidak dapat di update"})
		return
	}

	var d = models.DB.Model(&course).Where("Key = ?", 23)
	fmt.Println(d)
